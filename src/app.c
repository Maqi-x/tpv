...
void tpv_show_stats(TpvApp* app, const char* indent) {
    TimeSpanSec avg_typing_time = app->typing_times_sum / app->entered_words_count;
    TimeSpanSec avg_typing_time_per_char = app->typing_times_per_char_sum / app->entered_words_count;
    ...
    while (true) {
        ...
        if (app->args.time_limit.set && line.typing_time > app->args.time_limit.value) {
            ...
            printf(BOLD RED "%s" RESET " Exceeded time limit per character (%.2lfs > %.2lfs per char)\n",
                    tpv_get_random_retry_message(), line.typing_time, app->args.time_limit.value);
        } else if (!tpv_input_eql(input, text, app->args.ignore_case.value, app->args.ignore_punctuations.value)) {
            is_correct = false;
